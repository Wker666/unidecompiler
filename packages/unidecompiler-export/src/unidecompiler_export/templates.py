...
def _validate(request: TemplateRequest) -> None:
    ...
    elif request.include_simulation:
        raise TemplateExportError("simulation adapter generation is available only for frontend templates")
    if request.include_ai_guidance:
        if request.kind != "frontend":
...
def _render_tree(request: TemplateRequest, destination: Path) -> None:
    source_root = resources.files("unidecompiler_export").joinpath("assets", request.kind)
