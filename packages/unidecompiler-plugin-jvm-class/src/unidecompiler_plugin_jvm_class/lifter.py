...
JVM_EFFECT_TABLE = VMEffectTable(
    ...
    exact={
        **{opcode: _jvm_binary(opcode, op) for opcode, op in BINARY_OPS.items()},
        ...
        "checkcast": lambda _method, instruction, source: (CallTopAs(source=source, callee_name=f"checkcast<{instruction.operands}>"),),
        "instanceof": lambda _method, _instruction, source: (CallTopAs(source=source, callee_name="instanceof"),),
        "monitorenter": lambda _method, _instruction, source: (BuildCall(source=source, callee=Global(name="monitorenter", source=source), arg_count=1, returns=0),),
        "monitorexit": lambda _method, _instruction, source: (BuildCall(source=source, callee=Global(name="monitorexit", source=source), arg_count=1, returns=0),),
        ...
        "ret": _jvm_unsupported_control_effect,
        "i2l": lambda _method, _instruction, source: (CallTopAs(source=source, callee_name="i2l"),),
        **{opcode: (lambda _method, _instruction, source, _opcode=opcode: (CallTopAs(source=source, callee_name=_opcode),)) for opcode in {"i2d", "i2f", "i2b", "i2c", "i2s", "l2i", "l2f", "l2d", "f2i", "f2l", "f2d", "d2i", "d2l", "d2f"}},
